...
struct JointAttentionLogger::Impl {
    ...
    Impl(JointAttentionLogger &mod) : module(mod), fCallbackMutex(AL::ALMutex::createALMutex()) {
        ...
        try {
            memoryProxy->declareEvent("CallChild", "JointAttentionLogger");
            memoryProxy->declareEvent("EndSession", "JointAttentionLogger");
            memoryProxyRemote->subscribeToEvent("StartSession", "JointAttentionLogger", "onStartLogger");
            childCount = 0;
            callFinished = true;
        }
        catch (const AL::ALError& e) {
    ...
    void startLogger() {
        ...
        try {
            ...
            memoryProxyRemote->subscribeToEvent("ChildCalled", "JointAttentionLogger", "onChildCalled");
            memoryProxy->subscribeToEvent("EndSession", "JointAttentionLogger", "onStopLogger");
            memoryProxy->subscribeToEvent("KlasifikacijaGovoraEvent", "JointAttentionLogger", "onSoundClassified");
    ...
    void operator()() {
        ...
        while( true ) {
            ...
            try {
                ...
                if( sinceLastFace >= 5000 && sinceLastCall >= 5000 && callFinished && !end){
                    // Reset call finished
                    callFinished = false;
...
void JointAttentionLogger::onStartLogger() {
    ...
    impl->memoryProxyRemote->unsubscribeToEvent("StartSession", "JointAttentionLogger");
    ...
    impl->memoryProxyRemote->subscribeToEvent("ChildCalled", "JointAttentionLogger", "onChildCalled");
}
...
void JointAttentionLogger::onStopLogger(const std::string &key, const AL::ALValue &value, const AL::ALValue &msg) {
    ...
    AL::ALCriticalSection section(impl->fCallbackMutex);
    // Unsubscriptions
    impl->memoryProxy->unsubscribeToEvent("EndSession", "JointAttentionLogger");
    ...
    try {
        impl->memoryProxy->unsubscribeToEvent("FaceDetected", "JointAttentionLogger");
        impl->memoryProxyRemote->subscribeToEvent("StartSession", "JointAttentionLogger", "onStartLogger");
        impl->memoryProxy->unsubscribeToEvent("KlasifikacijaGovoraEvent", "JointAttentionLogger");
...
void JointAttentionLogger::onChildCalled(const std::string &key, const AL::ALValue &value, const AL::ALValue &msg) {
    ...
    AL::ALCriticalSection section(impl->fCallbackMutex);
    // Unsubscription
    impl->memoryProxyRemote->unsubscribeToEvent("ChildCalled", "JointAttentionLogger");
    // Update the time of the last call
...
void JointAttentionLogger::onSoundClassified(const std::string &key, const AL::ALValue &value, const AL::ALValue &msg) {
    ...
    // Subscribe back to the same event
    impl->memoryProxy->subscribeToEvent("KlasifikacijaGovoraEvent", "JointAttentionLogger", "onSoundClassified");
}
